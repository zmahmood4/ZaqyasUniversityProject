 ...
 *Zaqyas Ali Mahmood
 *Version 6 Final Code with Complete Comments
 *Some sections of the code which are repeated for 6 gates are not commented due to not being necassary 
*/
...
LiquidCrystal_I2C lcd(0x27, 16, 2); //mapping out the LCD length by width
...
void setup()
{
  ...
  lcd.clear(); //clear LCD 
  lcd.setCursor(0,0); //set the cursor to the first line and first charecter 
  lcd.print("Powering Up"); //message for the first line
  lcd.setCursor(0,1); //set the cursor to the second line and first charecter
  lcd.print("Please Wait..."); //message for the second line
  delay(2500);
  ...
  //test airfield LEDs by illuminating them all for 5 seconds at the start of the program
  //test status is displayed in the serial monitor and on the lcd display
  ...
  //LED illumination test is complete with visual inspection
  ...
  //in passive state the runway is clear for landing
  lcd.clear();
  lcd.setCursor(0,0);
  lcd.print("Runway Clear");
  lcd.setCursor(0,1);
  lcd.print("For Landing");
  //planes landing are directed to follow the reds towards the landing sensor
...
void loop()
{
  landing_takeoff(); //constant loop to call landing takeoff function function
...
{
  //the following sections of the airfield are turned off to allow only the passive sections to remain illuminated
  digitalWrite(s1, LOW);
  digitalWrite(s2, LOW);
  digitalWrite(s3, LOW);
  digitalWrite(s4, LOW);
